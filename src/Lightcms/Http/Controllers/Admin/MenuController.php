...
class MenuController extends Controller
{
    ...
    public function discovery ()
    {
        ...
        $modules = \collect (app ()->has ('modules') ? app ('modules')->all () : [])
            ...
            ->reduce (function ($arr, $item) use ($m_route_pfx)
            {
                ...
                $arr [ $route_pfx ] = Menu::updateOrCreate ([
                    ...
                    'name'       => '[' . _trans (
                            $item->getLowerName ()
                            . '::pages.app.menu.admin.'
                            . module_route_prefix ('.')
                            . $item->getLowerName ()
                            . '.index',
                            module_route_prefix ('-' . $item->getLowerName ())
                        ) . ']',
                    'guard_name' => 'admin',
                    'url'        => '/admin/' . module_route_prefix ('/') . $item->getLowerName (),
                    'remark'     => $item->getLowerName (),
                    'order'      => $item->get ('order', 0)
                ]);
                return $arr;
            }, \collect ([]));

        $modules_home = $modules->values ()->reduce (function ($arr, $menu)
        {
            $arr->push ($menu->route);
            return $arr;
        }, \collect ([]));
        ...
        foreach ($allRoutes as $k => $v) {
            if (Str::startsWith ($k, 'admin::') && ! $modules_home->contains ($k)) {
                ...
                try {
                    ...
                    module_tap ('laravellocalization', function ($laravellocalization) use (&$data)
                    {
                        $data [ 'url' ] = Str::replaceFirst (
